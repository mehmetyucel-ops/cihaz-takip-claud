/**
 * MEKANİK LABORATUVARI TAKİP SİSTEMİ
 * Tam Sürüm (Şık E-Posta Şablonları, Dış Lab İptali, İsimli Mail Bildirimleri, TDK İmla Düzeltmesi)
 */

const SPREADSHEET_ID = "1MQpi0pLpTs_NLsxAon5QmJa7qG5yt7BMaF7Wr9diWjM"; 
...
function clearLabCache() { CacheService.getScriptCache().remove("mekanik_lab_data"); }
...
function doGet(e) {
  ...
  return template.evaluate().setTitle('Mekanik Laboratuvarı').setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL).addMetaTag('viewport', 'width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no');
}
...
function adminAssignDevice(code, targetEmail, note) {
  ...
  <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e9ecef; border-radius: 10px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.05); background-color: #ffffff;">
      ...
      <div style="padding: 30px;">
          ...
          <p style="color: #555; font-size: 15px; line-height: 1.6;">Merhaba <b>${targetName}</b>,<br>Mekanik Laboratuvarı yöneticisi tarafından adınıza yeni bir cihaz zimmetlenmiştir.</p>
...
function transferToLab(code, labKey, note) {
  if (!checkIfAdmin(Session.getActiveUser().getEmail())) throw new Error("Yetkiniz yok!");
  const ssMek = getSpreadsheet(), invMek = ssMek.getSheetByName("Envanter"), dataMek = invMek.getDataRange().getValues();
  let rowIdx = -1, rowData = null, headersMek = dataMek[0];

  for (let i = 1; i < dataMek.length; i++) {
    if (String(dataMek[i][0]).trim() === String(code).trim()) { rowIdx = i + 1; rowData = dataMek[i]; break; }
  }
  if (rowIdx === -1) throw new Error("Cihaz envanterde bulunamadı!");

  let currentDurum = String(rowData[getDurumColIndex(invMek)-1]).trim();
  ...
  else if (labKey === "guc_elektronigi") { targetSS_ID = GUC_SS_ID; targetLabName = "Güç Elektroniği"; }
  else throw new Error("Geçersiz laboratuvar seçimi.");

  invMek.getRange(rowIdx, getDurumColIndex(invMek)).setValue(`Dış Lab. Devir Bekliyor:${targetLabName}`);

  let idxM = headersMek.findIndex(h => String(h).toLowerCase().includes("marka"));
  let idxMd = headersMek.findIndex(h => String(h).toLowerCase().includes("model"));
  ...
  for(let i=0; i<headersTarget.length; i++) {
     ...
     let mekIdx = headersMek.findIndex(h => String(h).trim() === hName);
     if (mekIdx !== -1) newRow[i] = rowData[mekIdx];
  ...
  let senderLabel = senderName + " (Mekanik Laboratuvarı)";

  let notCol = headersTarget.findIndex(h => h.toLowerCase().includes("arıza notu") || h.toLowerCase().includes("ariza notu"));
  if (notCol !== -1) newRow[notCol] = "[Mekanik Lab. Devri] " + (note || "");

  targetInv.appendRow(newRow); 

  let logSheet = ssMek.getSheetByName("Kayitlar");
  ...
  try {
      ...
      if(targetLogSheet) { 
          targetLogSheet.appendRow([String(code).trim(), String(rowData[1]).trim(), marka, model, senderLabel, logEmail, new Date(), new Date(), "", "Transfer Onayı Bekliyor", note || "Mekanik Laboratuvarından Transfer Edildi"]); 
      }
  } catch(e) {}
...
function cancelOutgoingTransfer(code) {
  ...
  if(durumVal.includes("Kalite") || durumVal.includes("Güç")) {
      let targetSS_ID = durumVal.includes("Kalite") ? KALITE_SS_ID : GUC_SS_ID;
      try {
...
function getLabData() {
  try {
    ...
    const cachedData = cache.get("mekanik_lab_data");
    let results = [];
    ...
    if (cachedData) {
        ...
        cache.put("mekanik_lab_data", JSON.stringify(results), 900);
    }
    return results;
...
function approveQuarantine(code, updatedData) { 
  ...
  let userEmail = Session.getActiveUser().getEmail(), userName = userEmail.split('@')[0].replace(/\./g, ' ').toUpperCase() + " (Mekanik Lab.)";
     ...
     if (currentDurum.includes("Biriminden Devir") || currentDurum.includes("Transfer Onayı Bekliyor")) {
          ...
          if(targetSS_ID) {
              try {
                  ...
                  if(tLog) tLog.appendRow([code, devName, details.marka, details.model, "Mekanik Lab.", userEmail, new Date(), new Date(), "", "Transfer Onaylandı", "Karşı laboratuvar cihazı kabul etti."]);
              } catch(e) {}
          }
...
// 📌 GÜNCELLENDİ: "Ret Sebebi" İmla Hatası Giderildi ve Loga Kişi İsmi Eklendi
function rejectQuarantine(code, rejectNote) {
  let userEmail = Session.getActiveUser().getEmail();
  let userName = userEmail.split('@')[0].replace(/\./g, ' ').toUpperCase() + " (Mekanik Lab.)";
  ...
  for (let i = 1; i < invData.length; i++) {
    if (String(invData[i][0]).trim() === String(code).trim()) {
      ...
      if (curDurum.includes("Biriminden Devir") || curDurum.includes("Transfer Onayı Bekliyor")) {
              ...
              try {
                  ...
                  for(let k=1; k<eData.length; k++) {
                      if(String(eData[k][0]).trim() === String(code).trim()) {
                          ...
                          // 📌 DÜZELTME: Karşı tarafın logunda "Ret Sebebi" ve Reddeden kişinin ismi yazacak
          ...
          invSheet.deleteRow(i + 1); clearLabCache(); return "Cihaz reddedildi ve gönderen laboratuvara iade edildi.";
...
function getLabStatistics() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (devName && userName && !userName.toLowerCase().includes("sistem") && !userName.toLowerCase().includes("mekanik")) {
